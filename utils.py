...
def get_net_config(config_path='NetConfig.json'):
    assert os.path.exists(config_path)
    with open(config_path, 'r') as f:
        return json.loads(f.read())


def get_num_workers(server_name, name_list_path='ServerList.json'):
    assert os.path.exists(name_list_path)
    with open(name_list, 'r') as f:
        name_list = json.loads(f.read())

    assert server_name in name_list
...
###Custom CosineAnnealing with Warm-up Restart
###Original Code: https://github.com/katsura-jp/pytorch-cosine-annealing-with-warmup/blob/master/cosine_annearing_with_warmup.py
